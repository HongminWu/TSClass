...
dist_clusts = np.load("Syn5/syn5_dist_cluster_original.npy")
clusters = np.load("Syn5/syn5_cluster_original.npy")
cluster_labels = np.load("Syn5/syn5_ycluster_original.npy")
...
############################# CHANGE THESE###############################
	...
	np.save("dist/syn5_distance_original_test"+str(num)+".npy",distancefeatures)
...
############################# CHANGE THESE###############################
	...
	np.save("dist/syn5_distance_original_train"+str(num)+".npy",dist_features_train)
#########################################################################
	'''
	for k in range(len(test_labels)):
	    if test_labels[k]>2:
